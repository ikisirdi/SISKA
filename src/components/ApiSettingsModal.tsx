...
import { X, Database, CheckCircle2, AlertTriangle, Link, RefreshCw, Key, MessageSquare, ExternalLink } from 'lucide-react';
...
export const ApiSettingsModal: React.FC<ApiSettingsModalProps> = ({
  ...
  const [fonnteToken, setFonnteToken] = useState(GASService.getFonnteToken());
  ...
  const handleSave = () => {
    ...
    GASService.setFonnteToken(fonnteToken.trim());
    onClose();
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg shadow-2xl text-slate-100 overflow-hidden my-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-bold text-base text-white">Pengaturan API Google & Fonnte WA</h3>
              <p className="text-xs text-slate-400">Hubungkan Dashboard dengan Google Sheets & WhatsApp Gateway</p>
            </div>
          </div>
        ...
        <div className="p-6 text-xs space-y-4">
          ...
          {/* Fonnte WhatsApp API Token Input */}
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs font-semibold text-emerald-400" htmlFor="input-fonnte-token">
                Token API Fonnte.com (WhatsApp Gateway)
              </label>
              <a
              ...
              >
                <span>Dapatkan Token di Fonnte.com</span>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Masukkan Token Fonnte (Contoh: xA9z1K...)"
                value={fonnteToken}
                onChange={(e) => setFonnteToken(e.target.value)}
                className="w-full pl-9 pr-3 py-2.5 text-xs font-mono rounded-xl border border-emerald-900/60 bg-emerald-950/20 text-emerald-200 focus:ring-2 focus:ring-emerald-500 outline-none"
              />
            </div>
            <p className="text-[11px] text-slate-400 mt-1">
              Token ini digunakan untuk pengiriman notifikasi WhatsApp otomatis dari server Google Apps Script dan pengujian langsung dari dashboard.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 pt-2">
            ...
            >
              ...
              <span>Tes Koneksi API Sheets</span>
            </button>
